...
ver:="1.97.1"
skipStartupDialog:=1	;1 - skip, 0 - show.
iniFile:="GGGlide.ini"
Gosub, GGGlideIcon
start:
...
DllCall("QueryPerformanceCounter", "Int64*", cT1), DllCall("GetCursorPos", "Ptr", lpP_addr), i:=mod(A_Index,3), ArrayX%i%:=vx:=10000000*((x1:= NumGet(lpP, 0, "Int"))-x0)/(cT1-cT0), ArrayY%i%:=vy:=10000000*((y1:= NumGet(lpP, 4, "Int"))-y0)/(cT1-cT0), Array%i%:=Round(Sqrt(vx**2+vy**2)), x0:=x1, y0:=y1, cT0:=cT1
}
...
DllCall("Sleep", "UInt", 1), DllCall("QueryPerformanceCounter", "Int64*", cT0), fff:= 1-Exp((cT0-cT1)*-0.072/507500)
